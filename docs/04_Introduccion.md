...
Tenemos que indicarle al usuario qué debe realizar, para realizar esto debemos utilizar el símbolo para enviar mensajes a la pantalla con el mensaje. *Los textos que queremos que se muestre debe ir entre doble comilla `" "`*. El mensaje que mostraremos es `"Dar el primer numero entero"`
...
<!-- text autogenerated-->
<blockquote style="" ><p><a href="./04_Introduccion.pdf" target="_blank">Descargar esta página en PDF</a></p> </blockquote>